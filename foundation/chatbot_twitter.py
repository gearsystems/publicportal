...
def gettype(rec_msg,new_msg,state,mode,complaint_id,stateslist):
	if mode%2 == 0:
		return "Give us a type from the following list[      ]",state,mode+1
	else:
		complaint=IncompleteComplaint.objects.get(id=complaint_id)
...
def getsuccess(rec_msg,new_msg,state,mode,statelist):
	#add the incomplete complaint to the complaint list
	print ''
